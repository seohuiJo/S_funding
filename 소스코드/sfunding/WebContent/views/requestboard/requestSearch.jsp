<%@ page import="requestboard.model.vo.RequestBoard" %>
            ...
            <!--메인 콘텐츠 위치-->
                <div class="row">
					...
					<div class="col-md-6 col-xl-6 d-flex justify-content-end">
						...
						<form action="/views/requestboard/requestWrite.jsp" method="post">
							<button type="submit" class="btn btn-primary">요청 글쓰기</button>
						</form>
						</c:if>
					</div>
                ...
                <div>
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <!--반복되는 테이블 데이터 출력부분-->
                        	<c:forEach items="${RequestPageData.pageList}" var="RequestServlet" varStatus="i">
                            <tr>
                                <th scope="row" class="text-center">${RequestServlet.requestNo}</th>
                                <td><a href="/requestSelect?requestNo=${RequestServlet.requestNo}" style="color:black;">${RequestServlet.requestTitle}</a></td>
                                <td>${RequestServlet.userId}</td>
                                <td class="text-center">${RequestServlet.rRegdate}</td>
                                <td class="text-center">${RequestServlet.good}</td>
                                <td class="text-center">${RequestServlet.bad}</td>
                                <td>${RequestServlet.projectList}</td>
                            </tr>
                        	</c:forEach>
                        </tbody>
                    </table>
