...
public class ServiceDAO {
	...
	public ArrayList<ServiceBoard> serviceSearchList(Connection conn, int currentPage, int recordCountPerPage, String search) {
		...
		String query = "SELECT * FROM (SELECT SERVICE_BOARD.*, ROW_NUMBER() OVER(ORDER BY SERVICE_NO DESC) AS NUM FROM SERVICE_BOARD WHERE SERVICE_CATEGORY LIKE '%"
	...
	public String getSearchPageNavi(Connection conn, int currentPage, int recordCountPerPage, int naviCountPerPage,
		...
		if (needPrev) {
			sb.append("<li class='page-item'><a class='page-link' style='color:black;' href='/serviceSearch?search=" + search + "&currentPage=" + (startNavi - 1) + "'><</a>></li>");
		}
		for (int i = startNavi; i <= endNavi; i++) {
			if (i == currentPage) {
				sb.append("<li class='page-item'><a class='page-link' style='color:black;' href='/serviceSearch?search=" + search + "&currentPage=" + i + "'><b>" + i + "</b></a></li>");
			} else {
				sb.append("<li class='page-item'><a class='page-link' style='color:black;' href='/serviceSearch?search=" + search + "&currentPage=" + i + "'>" + i + "</a></li>");
			}
		}

		if (needNext) {
			sb.append("<li class='page-item'><a class='page-link' style='color:black;' href='/serviceSearch?search=" + search + "&currentPage=" + (endNavi + 1) + "'>></a></li>");
		}
	...
	public ArrayList<ServiceComment> serviceCommentList(Connection conn, int serviceNo, int currentPage, int recordCountPerPage) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
